...
const SignUp = () => {
    ...
    // OTP VERIFICATION DISABLED - Direct login after signup
    /* useEffect(() => {
        // Check if signup was successful and requires OTP verification
        if (state.signup && (state.signup as any).requiresOtpVerification && (state.signup as any).userId) {
            const userId = (state.signup as any).userId;
            navigate(`/otp-verify?userId=${userId}`);
        }
    }, [state.signup, navigate]); */
